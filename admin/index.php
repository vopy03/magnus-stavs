...
if ($row_count == false && isset($_POST['enterf'])) {
	...
	$_SESSION['loginErrorMsg'] = $authIncorrectLogin;
}
else {
		if (password_verify($password, $pp)) {
			...
			$_SESSION['loginError'] = true;
			$_SESSION['loginErrorMsg'] = $authIncorrectPass;
	} 
}
...
if (isset($_POST['addProject'])) {
	...
	header('Location: index.php?la=proj');
}
...
if (isset($_POST['addReview'])) {
	...
	if(isset($_FILES['picture'])) {
		...
		header('Location: index.php?la=rev');
...
if (isset($_POST['updDataOnPage'])) {
	...
	header('Location: index.php?la=cont');
}
/*contacts end*/
...
if (isset($_POST['changeLang'])) {
	...
	header('Location: index.php?la=sett');
}
...
// if (isset($_POST['passwordHardRESET'])) {

// 	$newPass = "your_new_pass" 
...
<body>
	...
	<div class="admin-auth-form-container">
		<form class="admin-auth-form" method="POST" action="index.php">
			<?php if(@$_SESSION['loginError']) { ?>
			<div class="alert alert-warning"><span class="material-icons-outlined">report_problem</span><?php echo $_SESSION['loginErrorMsg'] ?></div>
		<?php } ?>
		  <div class="mb-3">
